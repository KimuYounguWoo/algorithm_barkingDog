...
//    if ( k != 0 ) index = arr[k-1];         //인덱스는... 전 인덱스를 받아야함.
    ...
    int check = 0;
    for(int i = index; i < n; ++i) {       // 1부터 n까지 / 중복 체크를 위해 후처리
        // 중복된 수열을 안 나오게 해야함/...
        if (!isused[i] && nums[i] != check) {
            arr[k] = nums[i];                 // k번째에 i를 담고 -> 여기서는 정수를 받기때문에 배열로 처리한다. -> arr이 들어가는 모든 값을 배열을 받게
            isused[i]=1;
            check = arr[k];
            cnt(k + 1, i+1);                // k+1번째로 재귀 반복 -> 더 이상 가능할 경우의 수가 없을 때까지 -> 조건에 맞춰 수열 출력 -> 이전을 바꿔야함.
            isused[i]=0;
        }
    }
}
